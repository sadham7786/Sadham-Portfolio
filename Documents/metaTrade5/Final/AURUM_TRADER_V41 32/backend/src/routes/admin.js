...
router.get('/simulation', asyncWrap(async (req, res) => {
  ...
  // Add human-readable speed label
  const speedLabel = _speedLabel(config.tickIntervalMs);
  res.json({ ...config, speedLabel });
}));
...
//   running        : true|false (pause/resume)
router.patch('/simulation', asyncWrap(async (req, res) => {
  const { tickIntervalMs, volatility, drift, running } = req.body;
  const updates = {};
  ...
  if (running         !== undefined) updates.running        = running;
