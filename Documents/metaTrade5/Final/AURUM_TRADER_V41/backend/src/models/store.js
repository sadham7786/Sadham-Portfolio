...
async function saveSimConfig(cfg) {
  if (mongoose.connection.readyState === 1) {
    await Config.findByIdAndUpdate('sim', { $set: {
      tickIntervalMs: cfg.tickIntervalMs,
      volatility:     cfg.volatility,
      drift:          cfg.drift,
      running:        cfg.running,
      simStatus:      cfg.simStatus,
      simDateMs:      cfg.simDateMs,
      simStartMs:     cfg.simStartMs,
      simEndMs:       cfg.simEndMs,
...
async function connectDB() {
  ...
  try {
    ...
    // Drop stale indexes left by older schema versions.
    // All collections now use _id (string UUID) as primary key.
    // Old schemas stored `id` and `userId` as regular fields with unique indexes,
    // causing E11000 dup key errors (null collisions) on every new document.
    const staleIndexes = [
      { col: 'users',         idx: 'id_1'     },
      { col: 'users',         idx: 'userId_1' },
      { col: 'trades',        idx: 'id_1'     },
      { col: 'trades',        idx: 'userId_1' },
      { col: 'pendingorders', idx: 'id_1'     },
      { col: 'pendingorders', idx: 'userId_1' },
      { col: 'transactions',  idx: 'id_1'     },
      { col: 'transactions',  idx: 'userId_1' },
    ];
    for (const { col, idx } of staleIndexes) {
      try {
        await mongoose.connection.collection(col).dropIndex(idx);
        console.log(`[db] Dropped stale index: ${col}.${idx}`);
      } catch (e) { /* index doesn't exist — safe to ignore */ }
    }
    await _loadCache();
  } catch (err) {
...
async function _loadCache() {
  ...
  // Load persisted simulation config
  try {
    const cfg = await Config.findById('sim').lean();
    if (cfg) {
      simConfig.tickIntervalMs = cfg.tickIntervalMs;
      simConfig.volatility     = cfg.volatility;
      simConfig.drift          = cfg.drift;
      simConfig.running        = cfg.running;
      if (cfg.simStatus)  simConfig.simStatus  = cfg.simStatus;
      if (cfg.simDateMs)  simConfig.simDateMs  = cfg.simDateMs;
      if (cfg.simStartMs) simConfig.simStartMs = cfg.simStartMs;
      if (cfg.simEndMs)   simConfig.simEndMs   = cfg.simEndMs;
      _startTick(); // restart tick with persisted config
      console.log(`[db] SimConfig loaded: tickIntervalMs=${cfg.tickIntervalMs} volatility=${cfg.volatility}`);
    }
  } catch (e) { console.warn('[db] SimConfig load error:', e.message); }
...
async function saveUser(user) {
  ...
  if (mongoose.connection.readyState === 1) {
    ...
    await User.findByIdAndUpdate(id, { $set: fields }, { upsert: true, new: true });
  }
}
...
async function saveTrade(trade) {
  ...
  if (mongoose.connection.readyState === 1) {
    ...
    await Trade.findByIdAndUpdate(id, { $set: fields }, { upsert: true, new: true });
  }
}
...
async function savePendingOrder(order) {
  ...
  if (mongoose.connection.readyState === 1) {
    ...
    await PendingOrder.findByIdAndUpdate(id, { $set: fields }, { upsert: true, new: true });
  }
}
...
async function saveTransaction(tx) {
  ...
  if (mongoose.connection.readyState === 1) {
    const { id, ...fields } = tx;
    await Transaction.findByIdAndUpdate(id, { $set: fields }, { upsert: true, new: true });
  }
}
...
const simConfig = {
  ...
  running:        true,
  basePrice:      3200.00,
  // Date-range simulation: Jan 1 2025 → Dec 31 2025
  // 'idle' = not started, 'running' = active, 'paused' = paused, 'stopped' = finished
  simStatus:  'running',                       // idle | running | paused | stopped
  simDateMs:  new Date('2025-01-01').getTime(), // current sim date (advances with ticks)
  simStartMs: new Date('2025-01-01').getTime(),
  simEndMs:   new Date('2025-12-31T23:59:59').getTime(),
};
...
function _startTick() {
  ...
  _tickTimer = setInterval(() => {
    ...
    // Advance simulation date — 1 tick = ~29ms real = proportional sim time
    // The full year (365d) completes based on tick speed. At 35 ticks/sec:
    // 365 days / (35 ticks/sec × 3600 sec/hr × 24 hr) ≈ 120ms per sim-day.
    if (simConfig.simStatus === 'running') {
      // Each tick advances sim time by (tickInterval / totalTicks) of the full range
      const totalMs = simConfig.simEndMs - simConfig.simStartMs;
      const totalTicks = 365 * 24 * 3600 * 1000 / simConfig.tickIntervalMs; // ticks in a year
      simConfig.simDateMs = Math.min(simConfig.simDateMs + (totalMs / totalTicks), simConfig.simEndMs);
      if (simConfig.simDateMs >= simConfig.simEndMs) {
        // Reached Dec 31 — auto-stop
        simConfig.simStatus = 'stopped';
        simConfig.running   = false;
        clearInterval(_tickTimer);
        _tickTimer = null;
        console.log('[sim] Date range completed — simulation stopped at Dec 31 2025');
      }
    }
...
function setSimConfig(updates) {
  ...
  // Date-range simulation controls
  if (updates.simStatus!==undefined) {
    const s = updates.simStatus;
    if (['idle','running','paused','stopped'].includes(s)) {
      simConfig.simStatus = s;
      simConfig.running   = (s === 'running');
      if (s === 'running' && simConfig.simDateMs >= simConfig.simEndMs) {
        // Reset to start when re-running after completion
        simConfig.simDateMs = simConfig.simStartMs;
      }
    }
  }
  if (updates.simDateMs!==undefined)  simConfig.simDateMs  = parseInt(updates.simDateMs);
  if (updates.simStartMs!==undefined) simConfig.simStartMs = parseInt(updates.simStartMs);
  if (updates.simEndMs!==undefined)   simConfig.simEndMs   = parseInt(updates.simEndMs);
  if (updates.tickIntervalMs!==undefined||updates.running!==undefined||updates.simStatus!==undefined) _startTick();
  // Persist to MongoDB so admin changes survive restarts
  saveSimConfig({...simConfig}).catch(()=>{});
  return {...simConfig};
}
