...
// In-memory watchlists (ephemeral — cleared on restart)
const _watchlists = new Map();

function _getWatchlist(userId) {
  return _watchlists.get(userId) || [...DEFAULT_WATCHLIST];
}
...
router.post('/', asyncWrap(async (req, res) => {
  ...
  _watchlists.set(req.userId, list);
...
router.put('/', asyncWrap(async (req, res) => {
  ...
  _watchlists.set(req.userId, symbols.map(s => String(s).toUpperCase()));
